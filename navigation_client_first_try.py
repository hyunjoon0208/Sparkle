...
class NavigationClient:
    ...
    def run(self):
        ...
        if self.client.get_state() != GoalStatus.ACTIVE:  # 현재
            ...
            range = 0.2
            if(self.curr_pose.pose.pose.position.x > self.goal_list[self.sequence].target_pose.pose.position.x - range and self.curr_pose.pose.pose.position.x < self.goal_list[self.sequence].target_pose.pose.position.x + range):
                self.end_flag = 1
            if (
