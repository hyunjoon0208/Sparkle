...
class PidCal:
    ...
    p2 = [0.00067, 0.000001, 0.00001]
    curve_p = [0.00215, 0.000001, 0.00001]
    curve_p2 = [0.009, 0.000001, 0.0003]
    ...
    def pid_control2(self, x_current, setpoint=320):
        self.x = int(x_current)
        self.twiddle()

        error = setpoint - x_current
        p1 = round(self.p2[0] * error, 9)
        self.error_sum += error
        i1 = round(self.p2[1] * self.error_sum, 9)
        d1 = round(self.p2[2] * (error -  self.error_old), 9)
        self.error_old = error
        pid = p1 +i1+ d1
        # pid = p1 + d1
        # pid = p1 + d1
