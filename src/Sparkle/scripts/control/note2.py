...
class main_drive:
    def __init__(self):
        ...
        self.back_wall_detected = False
        self.go_curve = False
        self.rotary_left = False #로터리 진입
        self.is_obstacle_mode_init = False
    ...
    def lidar_callback(self, msg):
        ...
        back_wall_degrees = [self.degrees[i] for i in range(len(self.scan.ranges)) if 6.6 < self.scan.ranges[i] < 10 and 178 < abs(self.degrees[i]) < 180]
        back_wall_degrees_arr = [self.scan.ranges[i] for i in range(len(self.scan.ranges)) if 6.6 < self.scan.ranges[i] < 10 and 178 < abs(self.degrees[i]) < 180]
        # front_wall_degrees = [self.degrees[i] for i in range(len(self.scan.ranges)) if 9.5 < self.scan.ranges[i] < 9.9 and abs(self.degrees[i]) < 5]
        # front_wall_degrees_arr = [self.scan.ranges[i] for i in range(len(self.scan.ranges)) if 9.5 < self.scan.ranges[i] < 9.9 and abs(self.degrees[i]) < 5]
        if back_wall_degrees:
            self.back_wall_detected = True
        else:
            self.back_wall_detected = False
        print("back: ", back_wall_degrees_arr)
        print("back_degree: ", back_wall_degrees)



        front_wall_degrees = [self.degrees[i] for i in range(len(self.scan.ranges)) if 9.5 < self.scan.ranges[i] < 10 and abs(self.degrees[i]) < 5]
        front_wall_degrees_arr = [self.scan.ranges[i] for i in range(len(self.scan.ranges)) if 9.5 < self.scan.ranges[i] < 10 and abs(self.degrees[i]) < 5]
        # front_wall_degrees = [self.degrees[i] for i in range(len(self.scan.ranges)) if 9.5 < self.scan.ranges[i] < 9.9 and abs(self.degrees[i]) < 5]
        # front_wall_degrees_arr = [self.scan.ranges[i] for i in range(len(self.scan.ranges)) if 9.5 < self.scan.ranges[i] < 9.9 and abs(self.degrees[i]) < 5]
        if self.flag == 2 and abs(self.yaw) > 0.998:
            if front_wall_degrees:
                self.front_wall_detected = True
            else:
                self.front_wall_detected = False
        # print(front_wall_degrees_arr)
        # print("front wall detected", self.front_wall_detected)
    ...
    def obstacle_mode_callback(self, msg):
        if self.slam_finish == False:
            return
        tmp_obstacle_mode = msg.data
        if self.obstacle_mode != 0 and tmp_obstacle_mode == 0:
            self.is_obstacle_mode_init = True
        self.obstacle_mode = tmp_obstacle_mode
        if self.obstacle_mode == 2 and self.static_obstacle2 == None and self.is_obstacle_mode_init == True: # 두번째 구간에서 정적 장애물 감지
            if self.static_obstacle != None:
                del self.static_obstacle
            self.static_obstacle2 = StaticObstacle()
            self.static_obstacle2.set_straight_yaw(-0.7)
            print("static obstacle222222222222222222222222222")
        elif self.obstacle_mode == 2 and self.is_obstacle_mode_init == False and self.static_obstacle == None: # 첫번째 구간에서 정적 장애물 감지
            self.static_obstacle = StaticObstacle()
            print("static obstacle111111111111111111111111111")
    ...
    def drive(self):
        ...
        if self.flag == 1: # 장애물 감지 및 회피
            if self.obstacle_mode == 0:
                ...
                if self.curve_detector.curve_flag == 0:
                    self.steer = self.static_obstacle.steer
                    self.speed = self.static_obstacle.speed
                elif self.curve_detector.curve_flag == 1:
                    self.steer = self.static_obstacle2.steer
                    self.speed = self.static_obstacle2.speed
            if self.curve_detector.curve_flag == 2:
                print("self.curve_detector.curve_flag == 2:")
                print("self.curve_detector.curve_flag == 2:")
                print("self.curve_detector.curve_flag == 2:")
                print("self.curve_detector.curve_flag == 2:")
            ...
            #     del(self.dynamic_obstacle)
                self.speed = 1200
                self.flag = 2
                if self.dynamic_obstacle != None:
                    del(self.dynamic_obstacle)
                if self.obstacle_mode != None:
                    del(self.obstacle_mode)
                if self.static_obstacle2 != None:
                    del(self.static_obstacle2)

        if self.flag == 2:
            self.steer = self.lane_steer
            print("yaw", self.yaw)

            if self.back_wall_detected == True:
                self.line_flag = 'CL'
                self.speed = 1200
                self.steer = self.lane_steer
        ...
        #     pass
            ...
            if self.go_curve == True:
                ...
                if 0.15 < abs(self.yaw - self.straight_yaw) < 0.4:
                    self.steer = 0.2
                elif 0.07 < abs(self.yaw - self.straight_yaw) <= 0.15:
                    self.steer = 0.32
                elif abs(self.yaw - self.straight_yaw) <= 0.06:
                    self.flag = 7
                    self.steer = 0.5
            ...
            #     self.flag = 8
            #     self.line_flag = 'CL'
            print("len(self.stop_line.stop_line):", len(self.stop_line.stop_line))
            ...
            if self.stop_line.stop_line_detected:
                self.line_flag = 'CR'
                self.rotary_left = True
                self.steer = self.lane_steer
                self.stop_line.stop_line_detected = False
            if 0.98 < self.yaw < 1.0:
                self.flag = 8
            
                


            # if 0.7 <= self.yaw < 0.72:
            #     self.line_flag = 'CR'
            #     self.steer = self.lane_steer
            #     self.stop_line.fast_stop_line_detected = False
