...
class Obstacle_mode:
    ...
    def lidar_callback(self, msg):
        ...
        degree_angle_increment = self.scan.angle_increment * 180 / pi
        degrees = [degree_min + degree_angle_increment * index for index, value in enumerate(self.scan.ranges)] 
        front_obstacle_degrees = [degrees[i] for i in range(len(self.scan.ranges)) if self.scan.ranges[i] <= 3.0 and 0 < degrees[i] < 70]
        # print("front_obstacle_distance", front_obstacle_degrees)
            ...
            if self.obstacle_mode == 0:
                if abs(self.straight_yaw - self.yaw) <= 0.1:
                    ...
                    # elif abs(degrees[index])< 15 and 0 < value < 1.6:  
                        ...
                        self.obstacle_mode = 2
                # elif 0.1 < abs(self.straight_yaw - self.yaw) < 0.3:
                #     print("detect!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                #     pass
