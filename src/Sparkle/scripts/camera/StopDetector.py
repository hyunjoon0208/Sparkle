...
class stop_line:
    ...
    def isStop(self,img):
        ...
        if img is not None:
            ...
            self.stop_line = ((nonzerox >= stop_left_x) & (nonzerox <= stop_right_x) & (nonzeroy >= stop_high_y) & (nonzeroy <= stop_low_y)).nonzero()[0]
            # out_img = cv2.polylines(out_img, [pts], True, (0,255,0), 1)
            print("stop_line_num : ", len(self.stop_line))
            if len(self.stop_line) > 12000:
