...
class YawDrive:
    ...
    def lidar_callback(self, msg):
        ...
        obstacle_degrees = [self.degrees[i] for i in range(len(self.scan.ranges)) if self.scan.ranges[i] < 1 and abs(self.degrees[i]) < 20]
        ...
        sideback_obstacle_degrees = [self.degrees[i] for i in range(len(self.scan.ranges)) if 90 < abs(self.degrees[i]) < 100 and self.scan.ranges[i] < 1]
        ...
        back_obstacle_degrees = [self.degrees[i] for i in range(len(self.scan.ranges)) if 155 < abs(self.degrees[i]) <= 180 and self.scan.ranges[i] < 3]
    ...
    #         print("left")
    #         steering = abs(pid - 0.5)
    #     if self.obstacle_detected or self.side_obstacle_detected or self.sideback_obstacle_detected or self.back_obstacle_detected:
    #         pass
    #     else:
    ...
    def avoid(self):
        ...
        if not self.sideback_obstacle_detected and self.avoid_state == 3:
            ...
            if abs(self.yaw - self.straight_yaw) > 0.1:
                ...
                self.steer = 0.5
                self.speed = 1000
        if self.back_obstacle_detected and self.avoid_state == 5:
            self.avoid_state = 0
